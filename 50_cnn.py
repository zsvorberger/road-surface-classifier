...
def build_config() -> TrainConfig:
    ...
    cfg = TrainConfig(
        ...
        gold_roads_per_class=_env_int("GOLD_ROADS_PER_CLASS", 150),
        ...
        # 2) LEARNING RATE & SCHEDULING
        # =========================
        base_lr=_env_float("LEARNING_RATE", 3e-4),
        max_lr=_env_float("MAX_LR", _env_float("LEARNING_RATE", 3e-4)),
        ...
        label_smoothing=_env_float("LABEL_SMOOTHING", 0.10),
...
_MASKS_CONTAINER_CLIENT = None
_MASK_CFG = None


def _init_mask_weight_worker():
    global _MASKS_CONTAINER_CLIENT, _MASK_CFG
    _MASK_CFG = CFG
    service = connect_patch_storage(_MASK_CFG)
    _MASKS_CONTAINER_CLIENT = service.get_container_client(_MASK_CFG.masks_container)


def _compute_mask_weight(row):
    masks_container = _MASKS_CONTAINER_CLIENT
    mask_path = str(row.mask_path)
    mask_blob_name = _blob_name_from_path(mask_path, _MASK_CFG.masks_container)
    try:
        ...
        return float(mask_arr.mean()) * float(getattr(row, "dataset_weight", 1.0))
...
def compute_mask_means(rows, service, cfg: TrainConfig):
    masks_container = service.get_container_client(cfg.masks_container)
    global _MASKS_CONTAINER_CLIENT, _MASK_CFG
    _MASKS_CONTAINER_CLIENT = masks_container
    _MASK_CFG = cfg
    cache_dir = cfg.mask_weight_cache_dir
    cache_dir.mkdir(parents=True, exist_ok=True)
    cache_key = pd.util.hash_pandas_object(
        rows[["mask_path", "dataset_weight"]], index=False
    ).sum()
    cache_path = cache_dir / f"mask_weights_{int(cache_key)}_{len(rows)}.npy"
    if cache_path.exists():
        return np.array(np.load(cache_path), dtype=np.float32)
    weights = []
    rows_list = list(rows.itertuples(index=False))
    if __name__ == "__main__":
        try:
            worker_count = multiprocessing.cpu_count()
            if worker_count < 1:
                raise RuntimeError("multiprocessing cpu_count returned invalid worker count")
            with multiprocessing.Pool(
                processes=worker_count,
                initializer=_init_mask_weight_worker,
            ) as pool:
                weights = pool.map(_compute_mask_weight, rows_list)
            for idx in range(cfg.mask_weight_log_every, len(rows_list) + 1, cfg.mask_weight_log_every):
                logger.info("Computed mask weights %d/%d", idx, len(rows_list))
        except Exception:
            for idx, row in enumerate(rows_list, start=1):
                weights.append(_compute_mask_weight(row))
                if idx % cfg.mask_weight_log_every == 0:
                    logger.info("Computed mask weights %d/%d", idx, len(rows_list))
    else:
        for idx, row in enumerate(rows_list, start=1):
            weights.append(_compute_mask_weight(row))
    ...
    np.save(cache_path, weights_arr)
    return weights_arr
...
def train_on_tiles(cfg: TrainConfig):
    ...
    val_roads = None
    if cfg.val_roads_file.exists():
        try:
            with open(cfg.val_roads_file, "r", encoding="utf-8") as handle:
            ...
            logger.info("Validation road set loaded from disk: %s", cfg.val_roads_file)
        except Exception as exc:
            logger.warning("Failed to load validation road set from %s: %s", cfg.val_roads_file, exc)
            val_roads = None
    if val_roads is None:
        gravel_split = int(len(gravel_roads) * (1.0 - cfg.val_split))
        paved_split = int(len(paved_roads) * (1.0 - cfg.val_split))
    ...
    road_train = road_ids_arr[train_idx]
    road_val = road_ids_arr[val_idx]
    y_eval = y_arr[eval_idx]
    road_eval = road_ids_arr[eval_idx]
    ...
    train_weights = compute_mask_means(train_rows, patch_service, cfg)
    val_weights = compute_mask_means(val_rows, patch_service, cfg)
    eval_weights = compute_mask_means(eval_rows, patch_service, cfg)

    train_rows = train_rows.copy()
    val_rows = val_rows.copy()
    eval_rows = eval_rows.copy()
    train_rows["sample_weight"] = train_weights
    val_rows["sample_weight"] = val_weights
    eval_rows["sample_weight"] = eval_weights
