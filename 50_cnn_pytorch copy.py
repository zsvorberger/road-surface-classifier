...
def compute_mask_means(rows, service, cfg: TrainConfig, split_hash: str, split_name: str):
    ...
    cache_path = cache_dir / f"mask_weights_{split_name}_{split_hash}_{int(cache_key)}_{len(rows)}.npy"
    if cache_path.exists() and not cfg.resplit:
        logger.info("Loaded cached mask weights from disk: %s", cache_path)
        return np.array(np.load(cache_path), dtype=np.float32)
    if cache_path.exists() and cfg.resplit:
        logger.info("Recomputing mask weights (reason: RESPLIT=1)")
    else:
        logger.info("Recomputing mask weights (reason: cache missing)")
    weights = []
