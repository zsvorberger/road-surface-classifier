...
class TrainConfig:
    ...
    use_class_cond_aug: bool
    class_aug_rot90_max_k: int
    class_aug_gravel_brightness_min: float
    class_aug_gravel_brightness_max: float
    class_aug_gravel_contrast_min: float
    class_aug_gravel_contrast_max: float
    class_aug_paved_brightness_min: float
    class_aug_paved_brightness_max: float
    class_aug_paved_contrast_min: float
    class_aug_paved_contrast_max: float
    ...
    focal_gamma: float
    loss_reduction: str
    use_road_consistency: bool
    road_lambda: float
    use_hard_mining: bool
    hard_mining_alpha: float
    max_mining_weight: float
    mining_warmup_epochs: int
    use_label_noise: bool
    noise_start_epoch: int
    noise_beta: float
    min_trust: float
    road_stability_threshold: float
...
def build_config() -> TrainConfig:
    ...
    cfg = TrainConfig(
        ...
        max_patch_attempts_per_road=_env_int("MAX_PATCH_ATTEMPTS_PER_ROAD", 240),
...
### NEW GRAVEL VS PAVED AUGMENTATION 2/5/2026
        ...
        use_class_cond_aug=_env_bool("USE_CLASS_COND_AUG", True),
        class_aug_rot90_max_k=_env_int("CLASS_AUG_ROT90_MAX_K", 1),
        class_aug_gravel_brightness_min=_env_float("CLASS_AUG_GRAVEL_BRIGHTNESS_MIN", 0.90),
        class_aug_gravel_brightness_max=_env_float("CLASS_AUG_GRAVEL_BRIGHTNESS_MAX", 1.10),
        class_aug_gravel_contrast_min=_env_float("CLASS_AUG_GRAVEL_CONTRAST_MIN", 0.90),
        class_aug_gravel_contrast_max=_env_float("CLASS_AUG_GRAVEL_CONTRAST_MAX", 1.10),
        class_aug_paved_brightness_min=_env_float("CLASS_AUG_PAVED_BRIGHTNESS_MIN", 0.97),
        class_aug_paved_brightness_max=_env_float("CLASS_AUG_PAVED_BRIGHTNESS_MAX", 1.03),
        class_aug_paved_contrast_min=_env_float("CLASS_AUG_PAVED_CONTRAST_MIN", 0.97),
        class_aug_paved_contrast_max=_env_float("CLASS_AUG_PAVED_CONTRAST_MAX", 1.03),

        # =========================
        # 2) LEARNING RATE & SCHEDULING
        # =========================
        base_lr=_env_float("LEARNING_RATE",2e-3),
        max_lr=_env_float("MAX_LR", _env_float("LEARNING_RATE",2e-3)),
        min_lr=_env_float("MIN_LR", 5e-4),
        ...
        betas=(
            _env_float("ADAM_BETA1", 0.85),
            _env_float("ADAM_BETA2", 0.999),
        ),
        momentum=_env_float("SGD_MOMENTUM", 0.90),
        weight_decay=_env_float("WEIGHT_DECAY", 3e-4),
        nesterov=_env_bool("SGD_NESTEROV", False),

        # =========================
        # 4) LOSS FUNCTION
        # NEW ROAD TOGETHERNESS LAMBA FUNCTION 2/5/2026
        # =========================
        loss_type=_env("LOSS_TYPE", "bce_with_logits"),
        label_smoothing=_env_float("LABEL_SMOOTHING", 0.15),
        ...
        use_road_consistency=_env_bool("USE_ROAD_CONSISTENCY", True),
        road_lambda=_env_float("ROAD_LAMBDA", 0.05),
        use_hard_mining=_env_bool("USE_HARD_MINING", False),
        hard_mining_alpha=_env_float("HARD_MINING_ALPHA", 0.5),
        max_mining_weight=_env_float("MAX_MINING_WEIGHT", 2.0),
        mining_warmup_epochs=_env_int("MINING_WARMUP_EPOCHS", 2),
        use_label_noise=_env_bool("USE_LABEL_NOISE", False),
        noise_start_epoch=_env_int("NOISE_START_EPOCH", 6),
        noise_beta=_env_float("NOISE_BETA", 0.3),
        min_trust=_env_float("MIN_TRUST", 0.5),
        road_stability_threshold=_env_float("ROAD_STABILITY_THRESHOLD", 0.75),
...
def augment_patch_class_cond(patch, cfg: TrainConfig, label_int: int):
    if random.random() < cfg.augment_hflip_prob:
        patch = patch[:, ::-1, :]
    if random.random() < cfg.augment_vflip_prob:
        patch = patch[::-1, :, :]
    rot_max = min(cfg.augment_rot90_max_k, cfg.class_aug_rot90_max_k)
    if rot_max > 0:
        rot_k = random.randint(0, rot_max)
        if rot_k:
            patch = np.rot90(patch, k=rot_k, axes=(0, 1))
    if int(label_int) == 1:
        brightness = random.uniform(
            cfg.class_aug_gravel_brightness_min,
            cfg.class_aug_gravel_brightness_max,
        )
        contrast = random.uniform(
            cfg.class_aug_gravel_contrast_min,
            cfg.class_aug_gravel_contrast_max,
        )
    else:
        brightness = random.uniform(
            cfg.class_aug_paved_brightness_min,
            cfg.class_aug_paved_brightness_max,
        )
        contrast = random.uniform(
            cfg.class_aug_paved_contrast_min,
            cfg.class_aug_paved_contrast_max,
        )
    patch = np.clip((patch - 0.5) * contrast + 0.5, 0.0, 1.0)
    patch = np.clip(patch * brightness, 0.0, 1.0)
    return patch
...
class PatchBlobDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.augment and self.cfg.augmentations_enabled:
            if self.cfg.use_class_cond_aug:
                patch = augment_patch_class_cond(patch, self.cfg, int(row.label_int))
            else:
...
def _apply_focal(loss_raw, logits, targets, cfg: TrainConfig):
    ...
    if cfg.loss_type.strip().lower() != "bce_with_logits":
        return loss_raw
    if logits.dim() == 2 and logits.size(-1) == 1 and targets.dim() == 1:
...
def _consistency_probs(logits, cfg: TrainConfig):
    loss_name = cfg.loss_type.strip().lower()
    if loss_name == "cross_entropy" and logits.dim() == 2 and logits.size(-1) >= 2:
        return torch.softmax(logits, dim=-1)[:, 1]
    return torch.sigmoid(logits).view(-1)


def _road_consistency_loss(probs: torch.Tensor, road_ids):
    groups = {}
...
def _agreement_rate(probs: np.ndarray, labels: np.ndarray, threshold: float) -> float:
    if probs.size == 0:
        return 0.0
...
class RoadStatsTracker:
    def __init__(self, ema_decay: float = 0.9):
        self.ema_decay = ema_decay
        self.stats = {}

    def update(self, road_stats: dict):
        for rid, stat in road_stats.items():
            prev = self.stats.get(rid)
            if prev is None:
                self.stats[rid] = {
            ...
            self.stats[rid] = {
                "agreement": decay * prev["agreement"] + (1.0 - decay) * stat["agreement"],
                "variance": decay * prev["variance"] + (1.0 - decay) * stat["variance"],
            }

    def is_stable(self, rid: str, agreement_threshold: float, var_threshold: float) -> bool:
        stat = self.stats.get(rid)
        if not stat:
            return False
        return stat["agreement"] >= agreement_threshold and stat["variance"] <= var_threshold
...
def train_one_epoch(model, loader, optimizer, scheduler, scaler, device, epoch, cfg: TrainConfig, log_every=None, road_tracker: Optional[RoadStatsTracker] = None):
    ...
    trust_weights_all = []
    loss_per_class = {0: [], 1: []}
    mining_per_class = {0: [], 1: []}
    trust_per_class = {0: [], 1: []}
    road_epoch = {}
    # Variance threshold for stable roads (small confidence variance)
    road_var_threshold = 0.02
    ...
    for batch_idx, (xb, yb, wb, road_ids) in enumerate(loader, start=1):
        ...
        # Print LR for first batch of epoch (unless scheduler steps per batch, in which case we'll print after step)
        if batch_idx == 1 and not (scheduler is not None and cfg.scheduler_step_unit.lower() == "batch"):
            print(f"[Epoch {epoch}] LR:", [pg["lr"] for pg in optimizer.param_groups])

        yb_loss = _apply_label_smoothing(yb, cfg) 
        class_w = _class_weight_tensor(yb.view(-1), cfg)
        # Ensure wb is per-sample (handle class-level weights)
        if wb.dim() == 1 and wb.numel() == 2:
            wb_sample = torch.where(
                yb >= cfg.label_positive_threshold,
                wb[1],
                wb[0],
            )
        else:
            wb_sample = wb
        wb_sample = wb_sample.view(-1)
        ...
        with torch.autocast(device_type=device.type, dtype=_amp_dtype(cfg), enabled=cfg.use_amp):
            ...
            loss_raw = loss_raw.view(-1)
            loss_for_mining = loss_raw
            if class_w is not None:
                loss_raw = loss_raw * class_w.view(-1)
            mining_weights = torch.ones_like(loss_raw)
            trust_weights = torch.ones_like(loss_raw)
            probs_for_stats = _consistency_probs(logits, cfg)
            if cfg.use_hard_mining and epoch >= cfg.mining_warmup_epochs:
                with torch.no_grad():
                    probs_det = probs_for_stats.detach().float().cpu().numpy()
                    yb_det = yb.detach().float().cpu().numpy().reshape(-1)
                    loss_det = loss_for_mining.detach().float().cpu().numpy().reshape(-1)
                    road_groups = {}
                    for i, rid in enumerate(road_ids):
                        road_groups.setdefault(rid, []).append(i)
                    road_hardness = {}
                    for rid, idxs in road_groups.items():
                        road_loss = float(loss_det[idxs].mean()) if idxs else 0.0
                        preds = (probs_det[idxs] >= cfg.threshold).astype(np.int32)
                        labels = (yb_det[idxs] >= cfg.label_positive_threshold).astype(np.int32)
                        disagree = float((preds != labels).mean()) if idxs else 0.0
                        road_hardness[rid] = road_loss * (1.0 + float(disagree))
                    mean_hardness = np.mean(list(road_hardness.values())) if road_hardness else 1.0
                    for i, rid in enumerate(road_ids):
                        h = road_hardness.get(rid, mean_hardness)
                        w = 1.0 + cfg.hard_mining_alpha * (h / (mean_hardness + 1e-6))
                        mining_weights[i] = min(w, cfg.max_mining_weight)
            if cfg.use_label_noise and epoch >= cfg.noise_start_epoch:
                with torch.no_grad():
                    probs_det = probs_for_stats.detach().float().cpu().numpy()
                    road_groups = {}
                    for i, rid in enumerate(road_ids):
                        road_groups.setdefault(rid, []).append(i)
                    road_consensus = {rid: float(probs_det[idxs].mean()) for rid, idxs in road_groups.items()}
                    for i, rid in enumerate(road_ids):
                        if road_tracker and road_tracker.is_stable(
                            str(rid),
                            cfg.road_stability_threshold,
                            road_var_threshold,
                        ):
                            disagree = abs(probs_det[i] - road_consensus.get(rid, probs_det[i]))
                            trust = max(cfg.min_trust, 1.0 - cfg.noise_beta * disagree)
                            trust_weights[i] = trust
            loss = (loss_raw * mining_weights * trust_weights * wb_sample).mean()
            if cfg.use_road_consistency:
                probs_for_consistency = _consistency_probs(logits, cfg)
                consistency_loss = _road_consistency_loss(probs_for_consistency, road_ids)
                loss = loss + cfg.road_lambda * consistency_loss
        ...
        if batch_idx % accum_steps == 0 or batch_idx == len(loader):
            ...
            if scheduler is not None and cfg.scheduler_step_unit.lower() == "batch":
                scheduler.step()
                if batch_idx == 1:
                    print(f"[Epoch {epoch}] LR after scheduler:", [pg["lr"] for pg in optimizer.param_groups])
        ...
        with torch.no_grad():
            ...
            loss_raw_mean = float(loss_raw.mean().item())
            loss_final_mean = float((loss_raw * mining_weights * trust_weights * wb_sample).mean().item())
            batch_loss_raw.append(loss_raw_mean)
            batch_loss_final.append(loss_final_mean)
            batch_acc.append(float((preds == yb_flat).float().mean().item()))
            conf = torch.maximum(probs_flat, 1.0 - probs_flat)
            batch_conf.append(float(conf.mean().item()))
            batch_class_counts.append(
                (
                    int((yb_flat < cfg.label_positive_threshold).sum().item()),
                    int((yb_flat >= cfg.label_positive_threshold).sum().item()),
                )
            )
            mining_weights_all.extend(mining_weights.detach().float().cpu().numpy().tolist())
            trust_weights_all.extend(trust_weights.detach().float().cpu().numpy().tolist())
            yb_np = yb_flat.detach().float().cpu().numpy()
            loss_np = loss_raw.detach().float().cpu().numpy()
            mining_np = mining_weights.detach().float().cpu().numpy()
            trust_np = trust_weights.detach().float().cpu().numpy()
            for lbl in (0, 1):
                mask = yb_np >= cfg.label_positive_threshold if lbl == 1 else yb_np < cfg.label_positive_threshold
                if mask.any():
                    loss_per_class[lbl].extend(loss_np[mask].tolist())
                    mining_per_class[lbl].extend(mining_np[mask].tolist())
                    trust_per_class[lbl].extend(trust_np[mask].tolist())
            probs_np = probs_flat.detach().float().cpu().numpy()
            labels_np = yb_flat.detach().float().cpu().numpy()
            for i, rid in enumerate(road_ids):
                rid = str(rid)
                entry = road_epoch.setdefault(rid, {"probs": [], "labels": [], "loss": [], "mining": [], "trust": []})
                entry["probs"].append(float(probs_np[i]))
                entry["labels"].append(int(labels_np[i] >= cfg.label_positive_threshold))
                entry["loss"].append(float(loss_np[i]))
                entry["mining"].append(float(mining_np[i]))
                entry["trust"].append(float(trust_np[i]))
    ...
    # ---- Per-epoch aggregated logging (no behavior change) ----
    if total_seen:
        mean_loss_raw = float(np.mean(batch_loss_raw)) if batch_loss_raw else 0.0
        mean_loss_final = float(np.mean(batch_loss_final)) if batch_loss_final else 0.0
        mean_acc = float(np.mean(batch_acc)) if batch_acc else 0.0
        mean_conf = float(np.mean(batch_conf)) if batch_conf else 0.0
        cls0 = sum(c[0] for c in batch_class_counts)
        cls1 = sum(c[1] for c in batch_class_counts)
        logger.info(
            "Epoch %d train batch stats: loss_raw=%.4f loss_final=%.4f acc=%.4f conf=%.4f class_dist=[paved=%d gravel=%d]",
            epoch,
            mean_loss_raw,
            mean_loss_final,
            mean_acc,
        ...
        mining_arr = np.array(mining_weights_all, dtype=np.float32) if mining_weights_all else None
        if cfg.use_hard_mining and mining_arr is not None:
            logger.info(
                "Epoch %d hard-mining: mean=%.4f max=%.4f pct>1=%.2f",
                epoch,
                float(mining_arr.mean()),
                float(mining_arr.max()),
                float((mining_arr > 1.0).mean() * 100.0),
            )
        trust_arr = np.array(trust_weights_all, dtype=np.float32) if trust_weights_all else None
        if cfg.use_label_noise and trust_arr is not None:
            logger.info(
                "Epoch %d label-noise: mean=%.4f min=%.4f pct<trust0.8=%.2f",
                epoch,
                float(trust_arr.mean()),
                float(trust_arr.min()),
                float((trust_arr < 0.8).mean() * 100.0),
            )
        # Distributions (min/median/p90) for mining/trust
        if mining_arr is not None:
            logger.info(
                "Epoch %d mining weight dist: min=%.3f p50=%.3f p90=%.3f max=%.3f",
                epoch,
                float(np.min(mining_arr)),
                float(np.median(mining_arr)),
                float(np.percentile(mining_arr, 90)),
                float(np.max(mining_arr)),
            )
        if trust_arr is not None:
            logger.info(
                "Epoch %d trust weight dist: min=%.3f p50=%.3f p90=%.3f max=%.3f",
                epoch,
                float(np.min(trust_arr)),
                float(np.median(trust_arr)),
                float(np.percentile(trust_arr, 90)),
                float(np.max(trust_arr)),
            )
        # Per-class averages
        for lbl, name in [(0, "paved"), (1, "gravel")]:
            if loss_per_class[lbl]:
                logger.info(
                    "Epoch %d class=%s avg_loss=%.4f avg_mining=%.4f avg_trust=%.4f",
                    epoch,
                    name,
                    float(np.mean(loss_per_class[lbl])),
                    float(np.mean(mining_per_class[lbl])) if mining_per_class[lbl] else 1.0,
                    float(np.mean(trust_per_class[lbl])) if trust_per_class[lbl] else 1.0,
                )
        # Road-level diagnostics
        road_stats = {}
        stable = mixed = suspicious = 0
        agg_patches = []
        agg_pred = []
        agg_var = []
        agg_agree = []
        ...
        agg_mining = []
        agg_trust = []
        for rid, data in road_epoch.items():
            probs = np.array(data["probs"], dtype=np.float32)
            labels = np.array(data["labels"], dtype=np.int32)
            if probs.size == 0:
                continue
            agreement = _agreement_rate(probs, labels, cfg.threshold)
            variance = float(np.var(probs))
            mean_pred = float(np.mean(probs))
            mean_loss = float(np.mean(data["loss"])) if data["loss"] else 0.0
            mean_mining = float(np.mean(data["mining"])) if data["mining"] else 1.0
            mean_trust = float(np.mean(data["trust"])) if data["trust"] else 1.0
            road_stats[rid] = {
                "agreement": agreement,
                "variance": variance,
                "mean_pred": mean_pred,
                "mean_loss": mean_loss,
                "mean_mining": mean_mining,
                "mean_trust": mean_trust,
                "patches": int(probs.size),
            }
            agg_patches.append(int(probs.size))
            agg_pred.append(mean_pred)
            agg_var.append(variance)
            agg_agree.append(agreement)
            agg_loss.append(mean_loss)
            agg_mining.append(mean_mining)
            agg_trust.append(mean_trust)
            if agreement >= cfg.road_stability_threshold and variance <= road_var_threshold:
                stable += 1
            elif agreement < 0.5:
                suspicious += 1
            else:
        ...
        if agg_patches:
            logger.info(
                "Epoch %d road stats: avg_patches=%.2f mean_pred=%.3f mean_var=%.4f mean_agree=%.3f mean_loss=%.4f mean_mining=%.3f mean_trust=%.3f",
                epoch,
                float(np.mean(agg_patches)),
                float(np.mean(agg_pred)),
                float(np.mean(agg_var)),
                float(np.mean(agg_agree)),
                float(np.mean(agg_loss)),
                float(np.mean(agg_mining)),
                float(np.mean(agg_trust)),
            )
        logger.info(
            "Epoch %d road categories: stable=%d mixed=%d suspicious=%d",
            epoch,
            stable,
            mixed,
            suspicious,
        )
        if road_tracker is not None:
            road_tracker.update(road_stats)
        _LAST_TRAIN_SUMMARY.update({
            "epoch": epoch,
            "mining_mean": float(mining_arr.mean()) if mining_arr is not None else None,
            "mining_max": float(mining_arr.max()) if mining_arr is not None else None,
            "mining_pct_up": float((mining_arr > 1.0).mean() * 100.0) if mining_arr is not None else None,
            "trust_mean": float(trust_arr.mean()) if trust_arr is not None else None,
            "trust_min": float(trust_arr.min()) if trust_arr is not None else None,
            "trust_pct_low": float((trust_arr < 0.8).mean() * 100.0) if trust_arr is not None else None,
            "roads_stable": stable,
            "roads_mixed": mixed,
            "roads_suspicious": suspicious,
            "roads_total": stable + mixed + suspicious,
            "road_avg_patches": float(np.mean(agg_patches)) if agg_patches else None,
        })
...
def train_on_tiles(cfg: TrainConfig):
    ...
    print("Initial LR(s):", [pg["lr"] for pg in optimizer.param_groups])
    ...
    road_tracker = RoadStatsTracker() if (cfg.use_label_noise or cfg.use_hard_mining) else None
    ...
    # Final run summary (non-duplicative, interpretive)
    final_epoch = cfg.epochs
    summary_epoch = best_epoch if best_epoch is not None else final_epoch
    agg_gain_val = road_acc - patch_acc
    agg_gain_eval = eval_road_acc - eval_patch_acc
    road_total = _LAST_TRAIN_SUMMARY.get("roads_total", 0) or 0
    stable_pct = (100.0 * _LAST_TRAIN_SUMMARY.get("roads_stable", 0) / road_total) if road_total else 0.0
    mixed_pct = (100.0 * _LAST_TRAIN_SUMMARY.get("roads_mixed", 0) / road_total) if road_total else 0.0
    suspicious_pct = (100.0 * _LAST_TRAIN_SUMMARY.get("roads_suspicious", 0) / road_total) if road_total else 0.0
    train_avg_patches = float(train_rows["road_id"].value_counts().mean()) if len(train_rows) else 0.0
    eval_avg_patches = float(sum(eval_road_counts) / len(eval_road_counts)) if eval_road_counts else 0.0

    logger.info("===== FINAL RUN SUMMARY =====")
    logger.info("Run id: epoch=%d checkpoint=%s", summary_epoch, cfg.model_out)
    logger.info("Aggregation gain (val): %.4f", agg_gain_val)
    logger.info("Aggregation gain (eval): %.4f", agg_gain_eval)
    if cfg.use_hard_mining and _LAST_TRAIN_SUMMARY.get("mining_mean") is not None:
        logger.info(
            "Hard mining: mean=%.3f pct>1=%.2f max=%.3f",
            _LAST_TRAIN_SUMMARY.get("mining_mean", 0.0),
            _LAST_TRAIN_SUMMARY.get("mining_pct_up", 0.0),
            _LAST_TRAIN_SUMMARY.get("mining_max", 0.0),
        )
    else:
        logger.info("Hard mining: inactive")
    if cfg.use_label_noise and _LAST_TRAIN_SUMMARY.get("trust_mean") is not None:
        logger.info(
            "Label noise: mean_trust=%.3f pct<trust0.8=%.2f min_trust=%.3f",
            _LAST_TRAIN_SUMMARY.get("trust_mean", 0.0),
            _LAST_TRAIN_SUMMARY.get("trust_pct_low", 0.0),
            _LAST_TRAIN_SUMMARY.get("trust_min", 0.0),
        )
    else:
        logger.info("Label noise: inactive")
    logger.info(
        "Road structure: avg_patches/train=%.2f avg_patches/eval=%.2f stable=%.1f%% mixed=%.1f%% suspicious=%.1f%%",
        train_avg_patches,
        eval_avg_patches,
        stable_pct,
        mixed_pct,
    ...
    if agg_gain_val > 0.0 and agg_gain_eval > 0.0:
        agg_msg = "Aggregation is improving road-level performance."
    else:
        agg_msg = "Aggregation gain is limited; patch-level signal may dominate."
    mining_msg = "Mining active but controlled." if (cfg.use_hard_mining and (_LAST_TRAIN_SUMMARY.get("mining_max", 0.0) <= cfg.max_mining_weight)) else "Mining inactive."
    noise_msg = "Noise modeling active; trust weights applied." if cfg.use_label_noise else "Noise modeling inactive."
    logger.info("Run diagnosis: %s %s %s", agg_msg, mining_msg, noise_msg)
