...
INDEX_OUTPUT = BASE_DIR / "output" / "patch_index.csv"
...
def main():
    ...
    if len(paved) == 0 or len(gravel) == 0:
        sys.exit("Gold dataset missing at least one class; cannot continue.")
    ...
    all_target_ids = set(sample_map.keys())
    ...
    road_to_tiles = naip_matches.groupby("id")["tile_url"].apply(list).to_dict()
    logger.info(
    ...
    with open(INDEX_OUTPUT, "w", newline="", encoding="utf-8") as index_handle:
        ...
        for idx, road_id in enumerate(gold_ids, start=1):
            ...
            logger.info("Processing gold road %d/%d (osm_id=%s)", idx, total_roads, road_id)
            _, total_index_rows = _process_road(road_id, label, tiles, "gold", total_index_rows)
